...
parser.add_argument("--output_dir", default="lexical_layer_model")
...
def preprocess_text(text: str) -> str:
    ...
    # keep Arabic + English + digits + placeholders
    text = re.sub(r"[^\w\s\u0600-\u06FF<>]", " ", text)

    # collapse heavy char repetition
    text = re.sub(r"(.)\1{2,}", r"\1", text)

    # lowercase English, normalize Arabic
    text = text.lower()
...
hard_df = load_csv(args.hard_file)
...
with open(OUT / "lexical_thresholds.json", "w", encoding="utf-8") as f:
    json.dump({
        ...
        "validation_grid": threshold_rows
